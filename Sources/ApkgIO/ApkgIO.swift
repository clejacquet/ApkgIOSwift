...
public struct AnkiPackage {
    ...
    static func parseCards(_ db: Connection, length: Int64? = nil, offset: Int64? = nil) throws -> [AnkiCard] {
        do {
            ...
            let queue = Expression<Int64>("queue")
            let due = Expression<Int64>("due")
            let ivl = Expression<Int64>("ivl")
            let factor = Expression<Int64>("factor")
            let reps = Expression<Int64>("reps")
            let lapses = Expression<Int64>("lapses")
            let left = Expression<Int64>("left")
    ...
    static func parseRevlog(_ db: Connection, length: Int64? = nil, offset: Int64? = nil) throws -> [AnkiRevlog] {
        do {
            ...
            let ease = Expression<Int64>("ease")
            let ivl = Expression<Int64>("ivl")
            let lastIvl = Expression<Int64>("lastIvl")
            let factor = Expression<Int64>("factor")
    ...
    static func parseCollections(_ db: Connection, is21: Bool) throws -> [AnkiCollection] {
        ...
        let decks = Expression<String>("decks")
